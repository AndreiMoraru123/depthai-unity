...
{
    public class DaiYoloDetector : PredefinedBase
    {
        ...
        * @returns pipeline
        */
        private static extern bool InitYoloDetector(in PipelineConfig config);
        ...
        */
        private static extern IntPtr YoloDetectorResults(out FrameInfo frameInfo, bool getPreview, float objectScoreThreshold, bool retrieveInformation, bool useIMU, int deviceNum);
        ...
        protected override bool InitDevice()
        {
            ...
            deviceRunning = InitYoloDetector(config);
        ...
        protected override void GetResults()
        {
            ...
            if (!device.replayResults)
            {
                ...
                objectDetectorResults = Marshal.PtrToStringAnsi(YoloDetectorResults(out frameInfo, GETPreview, detectionScoreThreshold, retrieveSystemInformation, useIMU, (int)device.deviceNum));
            }
        ...
        protected override void ProcessResults()
        {
            ...
            float bestPersonScore = 10000000.0f; ;
            foreach (JSONNode detection in obj["objects"])
            {
                // look for person and apple
                if (detection["label"] != "person") continue;
                if (!(detection["score"] < bestPersonScore)) continue;

                bestPersonScore = detection["score"];
                xmin = detection["xmin"];
                xmax = detection["xmax"];
                ymin = detection["ymin"];
                ymax = detection["ymax"];
            ...
            cubeCharacter.transform.localPosition = new Vector3((150 - (xmax - xmin) / 2) / 100.0f,
                                                                -((ymax - ymin) / 2 - 150) / 100.0f,
                                                                cubeCharacter.transform.localPosition.z);
